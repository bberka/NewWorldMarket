...
public class ItemImageOcrV3
{
    ...
    private readonly OcrInput _ocrInput;
    ...
    public ResultData<OcrReadResult> Read()
    {
        ...
        if (result.Pages.Length != 2) return Result.Fatal(ErrCode.OcrPageCountError);
        ...
        if (isNamed) itemRarity = itemRarity.Replace("Named", "").Trim();
        ...
        var itemType =
            firstPageLastLine.RemoveLineEndings().Replace(" ", "_").Trim(); //Replace for enum string matching
